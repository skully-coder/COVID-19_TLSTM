...
class LSTM(object):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, fc_dim, train):
        ...
        if train == 1:
            ...
            self.bf = self.init_bias(
                self.hidden_dim, name='Forget_Hidden_bias')
            ...
            self.bog = self.init_bias(
                self.hidden_dim, name='Output_Hidden_bias')
            ...
            self.b_softmax = self.init_bias(
                output_dim, name='Output_Layer_bias')
            ...
            self.bi = self.no_init_bias(
                self.hidden_dim, name='Input_Hidden_bias')
            ...
            self.bc = self.no_init_bias(
                self.hidden_dim, name='Cell_Hidden_bias')
            ...
            self.b_softmax = self.no_init_bias(
                output_dim, name='Output_Layer_bias')
    ...
    def LSTM_Unit(self, prev_hidden_memory, input):
        ...
        C = tf.nn.tanh(tf.matmul(x, self.Wc) +
    ...
    def get_states(self):
        ...
        packed_hidden_states = tf.scan(
            self.LSTM_Unit, scan_input, initializer=ini_state_cell, name='states')
        # h
